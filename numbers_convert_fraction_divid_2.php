...
for ($k=1; $k<=$number_of_questions; $k++)
{
	while (1)
	{
		$random_array = array(2,3,5,7,11);
		$randIndex = array_rand($random_array, 3);

		$number = $random_array[$randIndex[0]];
		$num = $random_array[$randIndex[1]];
		$den = $random_array[$randIndex[2]];
		
		$common_factor = rand(2,3);

		$number = $number * $common_factor;
		...
		$calculation_formula = sprintf('%s\\\\div\\\\frac{%s}{%s}',$number,$num,$den);
