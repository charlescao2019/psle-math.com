...
// time = alpha*beta;
	...
	$distance = $alpha * $gamma;
	$speed = $alpha * $beta;
	$speed_in_m_per_min_array = fraction2mixed($speed*1000,60,1,1);
	$speed_in_m_per_min_quotient = $speed_in_m_per_min_array[0];
	$speed_in_m_per_min_numerator = $speed_in_m_per_min_array[1];
	$speed_in_m_per_min_denumerator = $speed_in_m_per_min_array[2];
	if ($speed_in_m_per_min_denumerator==1)
		continue;

	if ($speed_in_m_per_min_quotient==0)
		continue;
    ...
    $question = sprintf('%s travelled at the speed of $%s\\\\frac{%s}{%s}$ m/min for $%s\\\\frac{%s}{%s}$ h. How many km did %s travel?', $name, $speed_in_m_per_min_quotient, $speed_in_m_per_min_numerator, $speed_in_m_per_min_denumerator, $time_quotient, $time_numerator, $time_denumerator, $name);
...
The Speed is $%s\\\\frac{%s}{%s}$ m/min = $\\\\frac{%s}{%s}$ m/min = $\\\\frac{\\\\frac{%s}{%s}}{\\\\frac{%s}{%s}}$ km/h = $\\\\frac{%s}{%s}\\\\times\\\\frac{%s}{%s}$ km/h = %s km/h, Time is $%s\\\\frac{%s}{%s}$ h, and therefore the distance is:
\\\\begin{array}{rcl}
%s\\\\times%s\\\\frac{%s}{%s}&=&%s\\\\times\\\\frac{%s}{%s}\\\\\\\\
&=&%s
\\\\end{array}
km.', $speed_in_m_per_min_quotient, $speed_in_m_per_min_numerator, $speed_in_m_per_min_denumerator, $speed_in_m_per_min_quotient*$speed_in_m_per_min_denumerator+$speed_in_m_per_min_numerator, $speed_in_m_per_min_denumerator, $speed_in_m_per_min_quotient*$speed_in_m_per_min_denumerator+$speed_in_m_per_min_numerator, 1000, $speed_in_m_per_min_denumerator, 60,$speed_in_m_per_min_quotient*$speed_in_m_per_min_denumerator+$speed_in_m_per_min_numerator, 1000, 60, $speed_in_m_per_min_denumerator, $speed, $time_quotient, $time_numerator, $time_denumerator, $speed,$time_quotient, $time_numerator, $time_denumerator,$speed,$time_quotient*$time_denumerator+$time_numerator,$time_denumerator,$distance);
