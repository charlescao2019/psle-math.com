...
for ($k=1; $k<=$number_of_questions; $k++)
{
    ...
    $randIndex = array_rand($num_array,4);
    ...
    $selected_fraction_indexes[3] = $num_array[$randIndex[3]];
    ...
    $selected_fraction_num_4 = ${sprintf('num_%s',$selected_fraction_indexes[3])};
    $selected_fraction_den_4 = ${sprintf('den_%s',$selected_fraction_indexes[3])};
    ...
    $mixed_fraction_4 = fraction2mixed($selected_fraction_num_4, $selected_fraction_den_4, rand(0,1));
	...
	$original_fraction_4 = sprintf('$%s\\\\frac{%s}{%s}$', $mixed_fraction_4[0],$mixed_fraction_4[1], $mixed_fraction_4[2]);
    ...
    echo $selected_fraction_num_4."/".$selected_fraction_den_4."\n\r";
    ...
    $associative_array = array(
        ...
        sprintf("%8.9f", $mixed_fraction_4[3]) => array($mixed_fraction_4[0],$mixed_fraction_4[1],$mixed_fraction_4[2])
    );
    ...
    if($ascending == 1)
    {
        $correct_answer = sprintf('%s', $fraction_0);
        $wrong_answer_1 = sprintf('%s', $fraction_3);
        $wrong_answer_2 = sprintf('%s', $fraction_1);
        $wrong_answer_3 = sprintf('%s', $fraction_2);
    }
    else
    {
        $correct_answer = sprintf('%s', $fraction_3);
        $wrong_answer_1 = sprintf('%s', $fraction_0);
        $wrong_answer_2 = sprintf('%s', $fraction_1);
        $wrong_answer_3 = sprintf('%s', $fraction_2);
    }
    ...
    if ($ascending == 1)
    {
    	$title = sprintf('Which one of the following fractions is the <b>smallest</b>?');
    }
    else
    {
    	$title = sprintf('Which one of the following fractions is the <b>largest</b>?');
    }
