...
for ($k=1; $k<=$number_of_questions; $k++)
{
	while (1)
	{
		...
		$corrent_num = $den_1*$den_2-$num_1*$den_2-$num_2*$den_1;
		$corrent_den = $den_1*$den_2;
		...
		$calculation_formula = sprintf('%s-\\\\frac{%s}{%s}-\\\\frac{%s}{%s}',"1",$num_1,$den_1,$num_2,$den_2);

		$title = sprintf('Find the value of $%s$.', $calculation_formula);
