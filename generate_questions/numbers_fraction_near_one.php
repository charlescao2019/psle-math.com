...
for ($k=1; $k<=$number_of_questions; $k++)
{
    ...
    $correct_answer = sprintf('$%s\\\\frac{%s}{%s}$', $mixed_fraction_4[0],$mixed_fraction_4[1], $mixed_fraction_4[2]);
	$wrong_answer_1 = sprintf('$%s\\\\frac{%s}{%s}$', $mixed_fraction_3[0],$mixed_fraction_3[1], $mixed_fraction_3[2]);
	$wrong_answer_2 = sprintf('$%s\\\\frac{%s}{%s}$', $mixed_fraction_2[0],$mixed_fraction_2[1], $mixed_fraction_2[2]);
	$wrong_answer_3 = sprintf('$%s\\\\frac{%s}{%s}$', $mixed_fraction_1[0],$mixed_fraction_1[1], $mixed_fraction_1[2]);

    if (rand(0,1)==1)
    {
    	$title = sprintf('Which of the following is nearest to %s?', $nearest_value);
    }
    else
    {
    	$title = sprintf('Find the number closest to %s?', $nearest_value);
    }
    //echo sprintf("(%s) %s\n\r",$k, $title);
