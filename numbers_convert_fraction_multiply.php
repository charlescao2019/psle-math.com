...
for ($k=1; $k<=$number_of_questions; $k++)
{
	while (1)
	{
		...
		$randIndex = array_rand($random_array, 1);

		$number = rand(2,9);
		$gcd = $random_array[$randIndex];

		$den = $gcd*$number;
		$num = rand(1,3);

		$number = $number*rand(2,9);

		$fraction_array = simplify($num,$den);
		$num = $fraction_array[0];
		$den = $fraction_array[1];

		$corrent_num = $num*$number;
		$corrent_den = $den;
		...
		$calculation_formula = sprintf('\\\\frac{%s}{%s}\\\\times%s',$num,$den,$number);
