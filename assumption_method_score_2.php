...
// T = W+beta*(W/alpha)+(S/alpha);
    ...
    $question = sprintf('%s attended a %s. %s answered all %s questions. For each correct answer, %s will get %s marks. However, for each wrong answer, %s will be deducted by %s mark(s). If %s scored %s marks in total, how many questions did %s answer wrongly?', $name, $exam, $name, $total_answer, $name, $mark_correct, $name, $mark_wrong, $name, $total_score, $name);

	$result = $number_of_wrong_answer;

    $wrong_result_1 = formatDecimal($number_of_correct_answer);
    $wrong_result_2 = formatDecimal($number_of_correct_answer+1);
    $wrong_result_3 = formatDecimal($number_of_correct_answer+2);
    $wrong_result_4 = formatDecimal($result+2);
    $wrong_result_5 = formatDecimal($result+1);
    $wrong_result_6 = formatDecimal($result-1);
    $wrong_result_7 = formatDecimal($number_of_correct_answer-2);
    $wrong_result_8 = formatDecimal($number_of_correct_answer-1);
